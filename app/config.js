...
function trainFilter(destination, array, city, province, filterSettings = {}) {
	//console.log(array)
	const gSettings = filterSettings.gOrD || {};
	const overnightSettings = filterSettings.overnight || {};
	const daytripSettings = filterSettings.daytrip || {};

	// 1. G/D settings
	const gEnabled = gSettings.enabled !== false; // default true
	let gPrefixes = ['G', 'D'];
	if (gSettings.prefixes) {
		if (Array.isArray(gSettings.prefixes)) {
			gPrefixes = gSettings.prefixes;
		} else if (typeof gSettings.prefixes === 'string') {
			gPrefixes = gSettings.prefixes.split(',').map(s => s.trim().toUpperCase()).filter(Boolean);
		}
	}

	// 2. Overnight settings (defaults: departBefore=1, departAfter=17, arriveBefore=14, arriveAfter=4, maxDuration=19)
	const overnightEnabled = overnightSettings.enabled !== false; // default true
	const overnightDepartBefore = parseFloat(overnightSettings.departBefore !== undefined ? overnightSettings.departBefore : 1) * 60;
	const overnightDepartAfter = parseFloat(overnightSettings.departAfter !== undefined ? overnightSettings.departAfter : 17) * 60;
	const overnightArriveBefore = parseFloat(overnightSettings.arriveBefore !== undefined ? overnightSettings.arriveBefore : 14) * 60;
	const overnightArriveAfter = parseFloat(overnightSettings.arriveAfter !== undefined ? overnightSettings.arriveAfter : 4) * 60;
	const overnightMaxDuration = parseFloat(overnightSettings.maxDuration !== undefined ? overnightSettings.maxDuration : 19) * 60;

	// 3. Daytrip settings (defaults: departAfter=7, arriveBefore=12, maxDuration=2)
	const daytripEnabled = daytripSettings.enabled !== false; // default true
	const daytripDepartAfter = parseFloat(daytripSettings.departAfter !== undefined ? daytripSettings.departAfter : 7) * 60;
	const daytripArriveBefore = parseFloat(daytripSettings.arriveBefore !== undefined ? daytripSettings.arriveBefore : 12) * 60;
	const daytripMaxDuration = parseFloat(daytripSettings.maxDuration !== undefined ? daytripSettings.maxDuration : 2) * 60;
	...
	array.map(function (obj) {
		const trainNo = obj.No || '';
		
		// 1. G/D match
		if (gEnabled) {
			const isMatch = gPrefixes.some(pref => trainNo.startsWith(pref));
			if (isMatch) {
				hasGOrD.push(trainNo);
			}
		}

		// 2. Overnight match
		if (overnightEnabled) {
			const dep = timeDefine(obj.depart);
			const arr = timeDefine(obj.arrive);
			const dur = timeDefine(obj.duration);

			// Note the operator precedence from original code: (depart < before) || (depart > after && arrive < before && arrive > after && duration < maxDuration)
			const isOvernight = dep < overnightDepartBefore || (dep > overnightDepartAfter && arr < overnightArriveBefore && arr > overnightArriveAfter && dur < overnightMaxDuration);
			if (isOvernight) {
				overNight.push(trainNo);
			}
		}
		...
		if (daytripEnabled) {
			...
			const isDaytrip = arr < daytripArriveBefore && dep > daytripDepartAfter && dur <= daytripMaxDuration;
			if (isDaytrip) {
				daytrip.push(trainNo);
			}
		}
	})
...
async function getBingFirstImage(keyword = '棋盘山') {
	...
	try {
		...
		await page.goto(`https://cn.bing.com/images/search?q=${encodeURIComponent(keyword)}`, {
			waitUntil: 'networkidle2',
			timeout: 30000
		});

		let targetImageUrl = await page.evaluate(() => {
			...
			for (let el of elements) {
				...
				if (mAttr) {
					try {
						...
						if (mJson && mJson.murl) {
							if (!mJson.murl.includes('sinaimg.cn')) {
								return mJson.murl;
							}
						}
					} catch (e) {
			...
			for (let img of mimgs) {
				if (img.src && img.src.startsWith('http') && !img.src.includes('sinaimg.cn')) {
					return img.src;
				}
			}
			return null;
		...
		if (!targetImageUrl || targetImageUrl === 'chrome-error://chromewebdata/') {
...
module.exports = {
	...
	outerURL: 'https://cdn.nextsticker.cn/',
	port: 4000,
